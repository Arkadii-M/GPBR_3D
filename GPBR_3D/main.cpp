...
int main()
{
	...
	//std::unique_ptr<IExpressionNode> cos = std::make_unique<UnaryNode>([](const arma::dmat& val) {return arma::cos(val); }, "cos", thetha);
	//std::unique_ptr<IExpressionNode> sin = std::make_unique<UnaryNode>([](const arma::dmat& val) {return arma::sin(val); }, "sin", phi);


	//std::unique_ptr<IExpressionNode> plus = std::make_unique<BinaryNode>([](const arma::dmat& x, const arma::dmat& y) {return x + y; }, "+", cos, sin);
	...
	//std::unique_ptr<IExpressionNode> exp = std::make_unique<UnaryNode>([](const arma::dmat& val) {return arma::cos(val); }, "exp", thetha1);
	//std::unique_ptr<IExpressionNode> tan = std::make_unique<UnaryNode>([](const arma::dmat& val) {return arma::sin(val); }, "tan", phi1);


	//std::unique_ptr<IExpressionNode> minus = std::make_unique<BinaryNode>([](const arma::dmat& x, const arma::dmat& y) {return x + y; }, "-", exp, tan);

	//std::cout << plus->toString() << "\n";

	//std::cout << minus->toString() << "\n";

	//std::unique_ptr<ExpressionTree> tree1 = std::make_unique<ExpressionTree>(plus);
	//std::unique_ptr<ExpressionTree> tree2 = std::make_unique<ExpressionTree>(minus);
